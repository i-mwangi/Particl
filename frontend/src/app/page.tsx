        ...
        <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: "36px" }}>
            ...
            <p style={{ fontSize: "16px", lineHeight: 1.7, color: "var(--text-secondary)", maxWidth: "640px", margin: "0 auto" }}>
              3D surfaces, vector fields, and true statistical box plots — medians,
              quartiles, means, and outliers — typeset by the agent and compiled to
              vector-sharp PDF. This page came from a single prompt.
            </p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 5fr) minmax(0, 4fr)", gap: "26px", alignItems: "center" }}>
            <div className="soft-card" style={{ padding: "18px", background: "#fdfaf2" }}>
              <img
                src="/viz-showcase.png"
                alt="Three figures generated by Particl: a 3D sine ribbon surface, a 3D vector field in a cube, and a statistical box plot with medians, means and outliers"
                style={{ width: "100%", height: "auto", display: "block", borderRadius: "8px" }}
              />
            </div>
            <div>
              <div className="soft-card" style={{ padding: "22px", marginBottom: "18px" }}>
                <p style={{ fontSize: "11px", letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--text-muted)", marginBottom: "10px" }}>
                  The prompt
                </p>
                <p className="mono" style={{ fontSize: "13px", lineHeight: 1.7, color: "var(--text-primary)" }}>
                  &quot;A visual plots showcase: a 3D ribbon of sin(x), a vector field
                  inside a cube, and a box plot comparing outcomes across three test
                  cycles.&quot;
                </p>
              </div>
              <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: "12px", marginBottom: "18px" }}>
                {[
                  "Box plots with real quartiles, whiskers, mean markers and outliers",
                  "3D surfaces and vector fields with sensible camera angles",
                  "Vector output — every figure stays sharp at any zoom",
                  "Refine any figure by talking to it",
                ].map((t) => (
                  <li key={t} style={{ display: "flex", gap: "10px", alignItems: "flex-start", fontSize: "14.5px", lineHeight: 1.55, color: "var(--text-secondary)" }}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginTop: "3px" }}>
              ...
              <div className="soft-card" style={{ padding: "0", overflow: "hidden" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "8px 14px", borderBottom: "1px solid var(--border)", background: "var(--bg-elevated)" }}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--text-secondary)" strokeWidth="1.8"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>
                  <span className="mono" style={{ fontSize: "11px", fontWeight: 600, color: "var(--text-primary)" }}>main.tex</span>
                  <span style={{ marginLeft: "auto", fontSize: "10px", color: "var(--text-muted)" }}>written by the agent</span>
                </div>
                <pre className="mono" style={{ margin: 0, padding: "14px", fontSize: "11px", lineHeight: 1.65, color: "var(--text-primary)", overflowX: "auto", background: "var(--bg-surface)" }}>
{`\\addplot[boxplot prepared={
    lower whisker=4, lower quartile=5.7,
    median=6.9, average=6.6,
    upper quartile=7.6, upper whisker=9,
  }, thick, fill=vizpink!10]
  coordinates {(2,2.4) (2,11.6)}; % outliers`}
                </pre>
              </div>
            </div>
          </div>

          {/* Gallery: more real compiled pages */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "26px", marginTop: "34px" }}>
            {[
              { src: "/showcase-diagrams.png", label: "Diagrams & circuits", sub: "Pie charts, quantum circuits and architecture diagrams from one prompt" },
              { src: "/showcase-textbook.png", label: "Textbook chapters", sub: "Lesson sections, objectives boxes, worked examples and drawn figures" },
            ].map((g) => (
              <div key={g.src}>
                <div className="soft-card" style={{ padding: "14px", background: "#fdfaf2" }}>
                  <img
                    src={g.src}
                    alt={g.label}
                    style={{ width: "100%", height: "auto", display: "block", borderRadius: "6px", border: "1px solid var(--border-subtle)" }}
                  />
                </div>
                <p style={{ fontSize: "14px", fontWeight: 600, color: "var(--text-primary)", margin: "12px 0 3px", textAlign: "center" }}>{g.label}</p>
                <p style={{ fontSize: "12.5px", color: "var(--text-secondary)", textAlign: "center" }}>{g.sub}</p>
              </div>
            ))}
          </div>
        </div>
